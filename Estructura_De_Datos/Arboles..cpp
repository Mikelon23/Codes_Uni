...
void Mayor(nodo *p){
	...
	cout<<"El mayor valor es: "<<maximo<<endl;
}
...
void Menor(nodo *p){
	...
	cout<<"El menor valor es: "<<minimo<<endl;
}
...
int NumHojas(nodo *raiz) {
    if (raiz == NULL) { 
        return 0;
    } else if (raiz->izq == NULL && raiz->der == NULL) { 
        return 1;
    } else { 
        return NumHojas(raiz->izq) + NumHojas(raiz->der);
    }
}

int Num_hijos(nodo* raiz) {
    if (raiz == NULL) 
	    return 0;
    if ((raiz->izq == NULL && raiz->der != NULL) || (raiz->izq != NULL && raiz->der == NULL)) {
        return 1 + Num_hijos(raiz->izq) + Num_hijos(raiz->der);
    }
    return Num_hijos(raiz->izq) + Num_hijos(raiz->der);
}
...
int main() 
{
	...
	{ 
		...
		cout << "\n8. Mayores y Menores del arbol"; 
		cout <<"\n9. Contar Nodos";
		cout <<"\n10. Contar Hojas";
		cout <<"\n11. Contar Nodos con Hijos";
		cout << "\n\nIngrese opcion: "; 
		cin >> op; 
		system("cls"); 
		switch(op) 
		{
			...
			case 7: 
				...
				break;
			case 8:
				cout<<"Los mayores y menores del Arbol "<<endl;
				Mayor(raiz);
				Menor(raiz);
				getch();
				break;
			...
			case 10:
				ch = NumHojas(raiz);
				cout<<"Numero de Hojas: "<<ch<<endl;
				getch();
				break;
			case 11:
				c1h=Num_hijos(raiz);
				cout<<"Nodos con 1 hijo: "<<c1h<<endl;
				getch();
				break;
